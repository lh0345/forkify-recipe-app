class SearchView {
  ...
  getQuery() {
    const query = this.#parentEl.querySelector('.search__field').value.trim().toLowerCase();
    return this.#allowedQueries.includes(query) ? query : null;
  }
  ...
  addHandlerSearch(handler) {
    this.#parentEl.querySelector('.search__field').addEventListener('input', function (e) {
      ...
      if (query) handler(query); // Call handler only if query matches the allowed list
    }.bind(this));
  }
}
